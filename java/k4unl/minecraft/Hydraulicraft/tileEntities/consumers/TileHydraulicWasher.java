...
public class TileHydraulicWasher extends TileHydraulicBase implements
	...
	private void doWash(){
		if(isWashing()){
			...
			if(maxWashingTicks != 0) tank.drain(Constants.MIN_REQUIRED_WATER_FOR_WASHER / maxWashingTicks, true);
			...
			for(int vert = -1; vert <= 1; vert++) {
				for(int depth = 0; depth <= 2; depth++)	{
					...
					if(block instanceof BlockInterfaceValve){
						TileInterfaceValve dummyTE = (TileInterfaceValve)getWorldObj().getTileEntity(x, y, z);
						dummyTE.setTarget(xCoord, yCoord, zCoord);
						if(fluidValve == null){
							fluidValve = dummyTE;
						}else{
